            ...
            <div className='w-full h-1/6 flex items-center ' key={index}>
              <div className='w-full h-full bg-red-500'>
                <ul className='w-full h-full bg-white shadow-lg py-2 flex items-center justify-evenly'>
                  <li className='flex items-center justify-center w-1/6'><img src={data?.Patient_profile?.profile_pic} className='w-12 h-12 rounded-full border border-1' alt="" /></li>
                  ...
                  <li className='flex items-center justify-center w-1/6'>
                    <div className='flex gap-2 '>
                      <button className={`${data?.appointment_status==="Accepted" ||data?.appointment_status==="Cancelled" ?"hidden":""}bg-green-500 p-1 px-2 text-sm text-white rounded-[5px] active:bg-green-400`} onClick={() => setData({ appointment_id:data?.id, status_val: "Accepted" })}>
                        Accept
                      </button>
                      <button className={`${data?.appointment_status==="Accepted" ||data?.appointment_status==="Cancelled"?"hidden":""}bg-yellow-500 p-1 px-2  text-sm text-white rounded-[5px] active:bg-yellow-400`} onClick={() => setData({ appointment_id:data?.id, status_val: "Cancelled" })}>
                        Cancel
                      </button>
                      <button className='bg-blue-500 p-1  text-sm text-white rounded-[5px] active:bg-blue-400'>
                        View
                      </button>
                    </div>
                  </li>
